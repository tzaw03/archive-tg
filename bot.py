    ...
    async def handle_callback(self, event):
        ...
        if data.startswith('format_'):
            ...
            try:
                ...
                artist_name = item_metadata.get('creator', 'Unknown Artist')
...
💾 Format: {format_name}
                ...
                # Send album info first
                await self.channel_handler.send_message(album_info)

                # Now upload tracks one by one (filename only)
                for i, file_info in enumerate(files):
                    ...
                    success = await self.channel_handler.upload_file(
                        file_stream, file_name, caption=None   # <-- no caption
                    )
                    file_stream.close()
